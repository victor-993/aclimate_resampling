...
class Resampling():
  ...
  def __init__(self,path,country,cores):
     self.path = path
  ...
  def preprocessing(self,prob_root,  output_root, forecast_period):
  ...
  def save_forecast(self,output_root, year_forecast, forecast_period, prob, seasons_range, base_years, station):
  ...
  def resampling_master(self):

    #station, input_root, climate_data_root, proba, output_root, year_forecast, forecast_period = 3
    ...
    prob_normalized = self.preprocessing(input_root, verifica, output_root, forecast_period)
