...
class TestCompleteData(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cores = 2
        self.test_path = os.path.join(self.root_data,self.country)
    ...
    def test_download_file_force_true(self):
        ...
        complete_data = CompleteData(self.start_date, self.country, self.test_path, cores=self.cores)
    ...
    def test_download_file_force_false(self):
        ...
        file_name = f"chirp.{self.start_date.strftime('%Y.%m.%d')}.tif.gz"
        url = f"http://data.chc.ucsb.edu/products/CHIRP/daily/{str(self.start_date.year)}/{file_name}"
        daily_downloaded_path = os.path.join(self.test_path,"inputs","prediccionClimatica", "daily_downloaded")
        file_path = os.path.join(daily_downloaded_path, file_name)
        complete_data = CompleteData(self.start_date, self.country, self.test_path, cores=self.cores)
        
        # Create an empty file to simulate an already downloaded file
        os.makedirs(daily_downloaded_path, exist_ok=True)
